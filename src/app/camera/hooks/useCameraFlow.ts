...
export type CameraFlowState = {
  ...
  predictionShape: string | null;
  confidencePercent: string | null;
};

export function useCameraFlow(): CameraFlowState {
  ...
  const mutation = useMutation<PredictionResponse, Error, File>({
    mutationFn: async (file: File) => {
      ...
      if (!payload || typeof payload.shape !== "string" || typeof payload.confidence !== "number") {
  ...
  const handleCapture = useCallback(() => {
    ...
    if (metrics.overallScore < MIN_OVERALL_SCORE || metrics.detectionScore < MIN_DETECTION_SCORE) {
  ...
  const nextOrientation: Orientation | null = useMemo(() => {
    if (step === "front") {
      return "left";
    }
    if (step === "left") {
  ...
  const captureButtonLabel = useMemo(() => {
    if (step === "front") {
      return "Ambil Gambar Tampak Depan";
    }
    if (step === "left") {
      return "Ambil Gambar Tampak Kiri";
    }
    return "Ambil Gambar Tampak Kanan";
  }, [step]);
  ...
  const metricsForViewport: MetricsDisplay = {
    overallPercent: liveMetrics ? Math.round(liveMetrics.overallScore * 100) : null,
    detectionPercent: liveMetrics ? Math.round(liveMetrics.detectionScore * 100) : null,
    centerPercent: liveMetrics ? Math.round(liveMetrics.centerScore * 100) : null,
    sizePercent: liveMetrics ? Math.round(liveMetrics.sizeScore * 100) : null,
  };
  ...
  const shouldShowPrediction = mutation.isSuccess && !displayError;
  ...
  return {
    ...
    predictionShape: prediction?.shape ?? null,
    confidencePercent,
  };
}
